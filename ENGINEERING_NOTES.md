...
The project targets .NET Framework 4.7.2 and WinForms, so Windows with Visual Studio 2022 is
the supported path and needs no special handling. Compiling it on Linux with the .NET SDK is
possible, and since the point of archiving was to leave the code untouched, the goal was to
do it without editing a single project file.

Three obstacles, in the order they appear:

**1. `MSB3644`: reference assemblies for v4.7.2 not found.** The SDK ships targeting packs
for .NET 8 only. Fixed by restoring `Microsoft.NETFramework.ReferenceAssemblies.net472` and
pointing `FrameworkPathOverride` at its `build/.NETFramework/v4.7.2` directory.

**2. `MSB4216`: `GenerateResource` cannot start an x86 task host.** Legacy-format projects
run this task out of process against `resgen.exe`, which does not exist here. Fixed by
overriding `GenerateResourceMSBuildArchitecture` and `GenerateResourceMSBuildRuntime` to
`CurrentArchitecture` and `CurrentRuntime`.

**3. `MSB3823` / `MSB3822`: non-string resources need serialization support.** The `.resx`
files embed the window icon, which is not a string. This requires
`GenerateResourceUsePreserializedResources` and a reference to `System.Resources.Extensions`.

The third one is the interesting one, because adding a `<Reference>` means editing the csproj.
MSBuild has a documented extension point for exactly this, `CustomAfterMicrosoftCommonTargets`,
which injects a props file at evaluation time. Putting that file outside the repository keeps
the project files untouched:

```xml
<!-- linux-build.props, kept outside the repo -->
<Project>
  <PropertyGroup>
  ...
  <ItemGroup>
    <Reference Include="System.Resources.Extensions">
      <HintPath>$(NUGET_PACKAGES)/system.resources.extensions/8.0.0/lib/netstandard2.0/System.Resources.Extensions.dll</HintPath>
...
Result: 0 warnings, 0 errors, an 88 KB PE32 assembly. `git status` stayed empty throughout.

What this does and does not prove: the source type-checks against the real 4.7.2 reference
assemblies. It says nothing about runtime behaviour. The output is a WinForms binary, it does
not run here, and none of the timing, rendering, or flush paths described above were exercised.
